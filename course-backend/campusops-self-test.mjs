...
export async function testCampusOps(baseUrl) {
  async function call(path, status, actor = 'coordinator-1', body, key, scenario = 'success') {
    const response = await fetch(`${baseUrl}${path}`, {
      ...
      headers: { Authorization: `Bearer ${process.env.COURSE_BACKEND_ACCESS_TOKEN}`, 'X-Course-Actor': actor,
  ...
  await assert.rejects(fetch(`${baseUrl}${actionPath}`, {
    ...
    headers: { Authorization: `Bearer ${process.env.COURSE_BACKEND_ACCESS_TOKEN}`, 'X-Course-Actor': 'coordinator-1',
  ...
  const malformed = await fetch(`${baseUrl}/v1/geocoding?q=zona`, { headers: { Authorization: `Bearer ${process.env.COURSE_BACKEND_ACCESS_TOKEN}`, 'X-Course-Actor': 'reporter-1', 'X-Course-Scenario': 'malformed' } });
